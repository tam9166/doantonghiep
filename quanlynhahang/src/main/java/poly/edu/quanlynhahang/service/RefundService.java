...
public class RefundService {
    ...
    public RefundTransaction confirmCompleted(Long refundId, String providerReference,
        ...
        // The refund row's identity is generated by the database and is therefore
        // monotonic/unique across restarts and concurrent admins. Legacy callers
        // may still provide a real provider reference; new UI leaves it blank.
        ...
        if (RefundStatus.COMPLETED.equals(refund.getStatus())) {
            ...
            if (reference == null || reference.isBlank()) {
                recorded = paymentTransactionRepository.findByAggregateTypeAndAggregateIdAndStatus(
                                aggregateType(refund), aggregateId(refund), PaymentTransactionStatus.SUCCESS).stream()
                        .filter(transaction -> isMatchingCompletedRefund(transaction, refund))
                        .findFirst()
                        .orElseThrow(() -> new ResponseStatusException(HttpStatus.CONFLICT,
                                "Không tìm thấy giao dịch đối soát cho lần hoàn tiền đã hoàn tất"));
            } else {
                recorded = paymentTransactionRepository.findByProviderTransactionId(reference)
                        .filter(transaction -> isMatchingCompletedRefund(transaction, refund))
                        .orElseThrow(() -> new ResponseStatusException(HttpStatus.CONFLICT,
        ...
        if (reference == null || reference.isBlank()) reference = generatedReference(refund);
    ...
    private boolean isMatchingCompletedRefund(PaymentTransaction transaction, RefundTransaction refund) {
        return PaymentDirection.REFUND.equals(transaction.getDirection())
                ...
                && aggregateType(refund).equals(transaction.getAggregateType())
                && aggregateId(refund).equals(transaction.getAggregateId())
                && nvl(refund.getAmount()).compareTo(nvl(transaction.getAmount())) == 0;
    }

    private String aggregateType(RefundTransaction refund) {
        return refund.getReservationId() != null ? "RESERVATION" : "ORDER";
    }

    private Long aggregateId(RefundTransaction refund) {
        return refund.getReservationId() != null ? refund.getReservationId() : refund.getOrderId().longValue();
    }
    ...
    private String generatedReference(RefundTransaction refund) {
        if (refund.getId() == null) {
            throw new ResponseStatusException(HttpStatus.CONFLICT,
                    "Không thể tạo mã giao dịch hoàn tiền khi giao dịch chưa có mã.");
        }
