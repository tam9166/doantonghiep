function e(e,t=`Mộc Vị Restaurant`){let n=document.getElementById(e);if(!n)throw Error(`Không tìm thấy nội dung in: ${e}`);let r=document.createElement(`iframe`);r.setAttribute(`title`,`Khung in hóa đơn`),r.setAttribute(`aria-hidden`,`true`),r.style.position=`fixed`,r.style.width=`0`,r.style.height=`0`,r.style.border=`0`,document.body.appendChild(r);let i=r.contentDocument,a=[...document.querySelectorAll(`link[rel="stylesheet"], style`)].map(e=>e.outerHTML).join(`
`);i.open(),i.write(`<!doctype html><html><head><meta charset="utf-8"><title></title>${a}<style>
  ...
  </style></head><body><main id="print-root"></main></body></html>`),i.close(),i.title=t,i.getElementById(`print-root`).appendChild(n.cloneNode(!0));let o=()=>setTimeout(()=>r.remove(),0);r.contentWindow.addEventListener(`afterprint`,o,{once:!0}),r.contentWindow.focus(),r.contentWindow.print(),setTimeout(o,3e4)}export{e as t};