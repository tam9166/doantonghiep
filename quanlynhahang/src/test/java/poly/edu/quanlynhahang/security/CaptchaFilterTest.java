...
class CaptchaFilterTest {
    ...
    @Test
    void eventBookingUsesItsDedicatedCaptchaAction() throws Exception {
        CaptchaVerifier verifier = mock(CaptchaVerifier.class);
        when(verifier.verify("captcha-token", "127.0.0.1", "event-booking-create")).thenReturn(true);
        CaptchaFilter filter = new CaptchaFilter(verifier);
        ...
        verify(verifier).verify("captcha-token", "127.0.0.1", "event-booking-create");
    }
}
